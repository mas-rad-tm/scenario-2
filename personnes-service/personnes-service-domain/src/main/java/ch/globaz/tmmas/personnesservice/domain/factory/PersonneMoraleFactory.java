...
public class PersonneMoraleFactory {
	...
	public static PersonnePhysique creerPersonne(CreerPersonneMoraleCommand commande, PersonneRepository personneRepository) throws PersonnesIncoherenceException {

		NssNonExistant regle = new NssNonExistant(personneRepository);

		if(regle.isSatisfiedBy(commande.getNss())){
			return new PersonnePhysique(new NSS(commande.getNss()),commande.getNom(),
